...
double calculate_c_max(const vector<Point> &points) {
    ...
    for (const auto &point : points) {
        c_max = max(c_max, static_cast<double>(abs(point.x)));
        c_max = max(c_max, static_cast<double>(abs(point.y)));
...
bool affine_dependent_check(vector<Point> &A) {
    ...
    bool result = abs(affine_checker.gradient * A[2].x + affine_checker.c_value - A[2].y) < error_margin; //0.0000000001
    return result;
}
