...
int main() {
    ...
    if (!outfile) {
        cerr << "Error opening file for debug" << endl;
        return 1;
    }



    loadPoints("testData/dataset1_origin_and_axes.txt", points);
    ...
    debug << "center: " << intermediate_center <<  endl;
    debug << "max: " << three_points[0] << endl;
    debug << "radius max: "<< three_points[0].segment() << endl;
    debug << "2nd max: "<< three_points[1] << endl;
    debug << "radius max: "<< three_points[1].segment() << endl;
    ...
    for (int i = 0; i < sorted_coordinate_placeholder.size(); i++){
        ...
        if (intersection.segment() <= closest && none_is_similar && !affine_dependent ){
            debug << "check closest(3rd) " << sorted_coordinate_placeholder[i] << endl;
            closest = intersection.segment();
            ...
            debug << "closest! (3rd)" << closest << endl;
            max3 = sorted_coordinate_placeholder[i];
            new_center = intersection;
            debug << "intersection (3rd): " << intersection << endl;
        }
    }
